...
def load_resnet18(CONFIG):
    model = models.resnet18(pretrained=False)
    num_ftrs = model.fc.in_features
    # modifying the fully connected layer for CIFAR-100 
    model.fc = nn.Sequential(
        nn.Linear(num_ftrs, 512),
        nn.ReLU(),
        nn.Dropout(0.5), # Add dropout layer to prevent overfitting since resnet18 is a larger model
        nn.Linear(512, 100)
    )
...
def main():
    ...
    CONFIG = {
        ...
        "learning_rate": 0.001,
        "epochs": 20,  # Train for longer in a real scenario - using 20 epochs since resnet18 is a larger model that will take time to converge
    ...
    ############################################################################
    model = load_resnet18(CONFIG)   # instantiate your model 
    ...
    optimizer = optim.Adam(model.parameters(), lr=CONFIG["learning_rate"], weight_decay=1e-4) #Use Adam optimizer
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=CONFIG["epochs"]) #Use CosineAnnealingLR scheduler
