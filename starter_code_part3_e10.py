...
################################################################################
def train(epoch, model, trainloader, optimizer, criterion, CONFIG, use_mixup=False, use_cutmix=False, mixup_alpha=1.0):
    ...
    for i, (inputs, labels) in enumerate(progress_bar):
        ...
        if use_mixup:
            ...
            inputs, targets_a, targets_b, lam = cutmix_data(inputs, labels, mixup_alpha, use_cuda=True)
            optimizer.zero_grad()
            outputs = model(inputs)
            loss = mixup_criterion(criterion, outputs, targets_a, targets_b, lam)
...
def main():
    ...
    for epoch in range(CONFIG["epochs"]):
        train_loss, train_acc = train(epoch, model, trainloader, optimizer, criterion, CONFIG, use_mixup=True, mixup_alpha=1.0)
